...
def run_logistic_regression_posthoc(config, df_DL_preds):
    """
    ...
    df.loc[:, patient_id_col] = df.loc[:, patient_id_col].astype(int).apply(
        lambda x: '%0.{}d'.format(patient_id_length) % x)
