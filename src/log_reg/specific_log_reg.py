...
def load_CITOR_features(config,relevant_columns, train_data, val_data, toxicity):
    label = config['predefined_LRM']['CITOR'][toxicity]['label']
...
def CITOR_refit(config, toxicity = 'sticky_saliva_late'):
    ...
    if len(config['predefined_LRM']['CITOR'][toxicity]['models'].keys()) >1:
    ...
    for model_name in model_names:
       # select the columns per submodel
        relevant_columns = np.array(config['predefined_LRM']['CITOR'][toxicity]['models'][model_name]['features'])
        # train the submodel
...
def load_RP_features(config,relevant_columns, train_data, val_data, toxicity):
    label = config['predefined_LRM']['detox_RP'][toxicity]['label']
    X_train = train_data[relevant_columns][train_data[label] != -1]
    y_train = train_data[label][train_data[label] != -1]
    X_val = val_data[relevant_columns][val_data[label] != -1]
    y_val = val_data[label][val_data[label] != -1]    
...
def RP_refit(config, toxicity = 'niezink_model'):
    
    train_data, val_data = lr_load_data(config)
    
    if len(config['predefined_LRM']['detox_RP'][toxicity]['models'].keys()) >1:
        feature_dict = {'model1':{},'model2':{}}
        model_names = ['model1','model2']
    else:
        feature_dict = {'model1':{}}
    ...
    for model_name in model_names:
       # select the columns per submodel
        relevant_columns = np.array(config['predefined_LRM']['detox_RP'][toxicity]['models'][model_name]['features'])
        # train the submodel
        X_train, y_train, X_val, y_val = load_CITOR_features(config,relevant_columns, train_data, val_data, toxicity)
        model, auc = lr_fit_model(config, X_train, y_train, X_val, y_val)
