...
class GradCamPlusPlus(LayerAttribution, GradientAttribution):
    # NOTE: Daniel: this is a slighly hacked version of the "LayerGradCam" class from captum (which is just normal GradCam)
    # a slight adjustment allows us to use GradCam++ instead of GradCam, the edit is noted below.
    r"""
    ...
    def attribute(
        ...
        plusplus: bool = False,                      # NOTE: Daniel: setting this to True will use GradCam++
        ...
        Args:
            ...
            attr_dim_summation (bool, optional): Indicates whether to
                        ...
                        Default: True
            plusplus (bool, optional): Indicates whether to apply the GradCAM++ or
                        the normal GradCAM algorithm. The GradCAM++ algorithm
                        uses a weighted combination of the first and second order
                        gradients to compute the attributions. The default (False)
                        means to use the normal GradCAM algorithm.
        ...
        if not plusplus:            # NOTE: Daniel: this is the method to accumulate the gradients for 'normal' GradCam
            summed_grads = tuple(
                torch.mean(
                    layer_grad,
        ...
        else:                     # NOTE: Daniel: this is the only change from the original LayerGradCam class, which makes it GradCam++ 
        ...
        # NOTE: the rest is the same as the original LayerGradCam class
