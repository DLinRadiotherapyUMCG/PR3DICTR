...
class MultiTox_Loss_ORIGINAL(nn.Module):
    """
...
class MultiTox_Loss(nn.Module):
    ...
    def forward(self, outputs_dict, labels_dict):
        ...
        if num_valid_labels == 0: # in case all labels are missing, the loss will be `nan`, so we return 0 instead
            ...
            endpoint_list = self.config['columns']['labels']
            batch_loss_dict = {}
            mean_losses = []  # To store mean loss per class
            ...
            if len(endpoint_list) > 1:  # If there are multiple endpoints
                for idx, endpoint in enumerate(endpoint_list):
                    ...
                    if n_non_missing == 0:  # If there are no non-missing labels, set the loss to 0
                        batch_loss_dict[endpoint] = torch.tensor(0.0, device=predictions.device, dtype=predictions.dtype)
                        mean_losses.append(torch.tensor(0.0, device=predictions.device, dtype=predictions.dtype))
                    else:
                        mean_loss = batch_loss[:, idx].sum() / mask[:, idx].sum()
                        batch_loss_dict[endpoint] = mean_loss
                        mean_losses.append(mean_loss)
            else:  # If there is only one endpoint
                mean_loss = batch_loss[:].sum() / mask[:].sum()
                batch_loss_dict[endpoint_list[0]] = mean_loss
                mean_losses.append(mean_loss)

            # Total loss is the average of the mean loss per class
            batch_loss_mean = torch.stack(mean_losses).mean()
            batch_loss_mean = torch.clamp(batch_loss_mean, min=0, max=10000)  # Optional clamping
