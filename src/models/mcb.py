...
class CompactBilinearPoolingFn(Function):
    ...
    def forward(ctx, h1, s1, h2, s2, output_size, x, y, force_cpu_scatter_add=False):
        ...
        re = torch.fft.irfft(torch.stack((re_prod, im_prod), re_prod.dim()), dim=1, n=output_size)
    ...
    def backward(ctx,grad_output):
        ...
        grad_fx = torch.fft.irfft(torch.stack((grad_re_fx,grad_im_fx), grad_re_fx.dim()), dim=1, n=ctx.output_size)
        ...
        grad_fy = torch.fft.irfft(torch.stack((grad_re_fy,grad_im_fy), grad_re_fy.dim()), dim=1, n=ctx.output_size)
