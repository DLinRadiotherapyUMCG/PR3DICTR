...
class DCNN_LReLU(torch.nn.Module):
    ...
    def __init__(self, channels, depth, height, width, n_features, num_classes, filters, kernel_sizes, strides,
                 pad_value, n_down_blocks, lrelu_alpha, dropout_p, perform_pooling,
                 linear_units,
                 clinical_variables_position,
                 clinical_variables_linear_units,
                 clinical_variables_dropout_p,
                 use_bias=False, pooling_conv_filters=None, **kwargs):
    ...
    def forward(self, x, clinical_features):
        ...
        if (self.n_features > 0) and (self.clinical_variables_linear_units is not None):
            for layer in self.clinical_variables_layers:
                clinical_features = layer(clinical_features)
