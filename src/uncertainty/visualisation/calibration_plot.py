...
def plot_calibration_subplot(ax, df_UQ_temp, endpoint, ENDPOINT_TYPES, UQ_metrics_list, N_bins, colours_dict):
    ...
    # from src.evaluation.metrics.utils import threshold
    ...
    # thresh_value = 0.5
    # #thresh_value = threshold(config_temp, true_labels, mean_preds)
...
def plot_error_calibration_subplot(ax, df_UQ_temp, endpoint, ENDPOINT_TYPES, UQ_metrics_list, N_bins, colours_dict):
    ax.plot([0, 1], [0, 1], linestyle='--', color='gray', label='Ideal Calibration')
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_aspect('equal', adjustable='box')
...
def plot_calibration_error_over_dataset_size(
    ...
    for N_patients in N_patients_list:
        ...
        for UQ_metric_name in UQ_metrics_list:
            ...
            value = np.sum((np.array(aucs) - np.array(bin_centers))) / len(aucs)
            # COMPUTE METRIC HERE
