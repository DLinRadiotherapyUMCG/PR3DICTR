...
def calc_bins(y_true, y_pred, num_bins = 10, bin_type="fixed"):
    ...
    Bins can be either `fixed` (ECE) or `adaptive` (ACE), where `fixed` bins are equally spaced and `adaptive` bins are equally populated.
    """
...
def compute_mse(config, y_pred_list, y_true_list):
...
CLASSIFICATION_METRICS_DICT = {
    ...
    "recall" : recall_score,   # recall is the same as sensitivity
    "specificity" : lambda y_true, y_pred : recall_score(y_true, y_pred, pos_label=0),
    "f1" : f1_score,
}

def calculate_J_statistic_threshold(y_true, y_pred):
...
for fold_folder in Fold_folders:
    for metric_name, metric_func in CLASSIFICATION_METRICS_DICT.items():

        RESULTS_DICT[fold_folder][metric_name] = {}
        #print(model, metric_name, dataset)                
        for endpoint in config.endpoint_list:

            labels = RESULTS_DICT[fold_folder]["ensemble labels"][endpoint]
            preds = RESULTS_DICT[fold_folder]["ensemble preds"][endpoint]

            pred_threshold = calculate_J_statistic_threshold(labels, preds)
            binarised_preds = preds > pred_threshold

            metric_value = metric_func(labels, binarised_preds)
            RESULTS_DICT[fold_folder][metric_name][endpoint] = round(metric_value, 2)
