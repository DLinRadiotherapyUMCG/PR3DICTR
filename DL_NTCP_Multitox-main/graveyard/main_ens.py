...
from utils.ray_functions import BasicTrialLogger, TrialTerminationReporter, trial_str_creator
...
from utils.calibration_plots import make_calibration_plots
...
## TODO: one of these functions for data preprocessing?
def initialize(config, device, create_folders=True):
    ...
    if create_folders:
        ...
        if not config.hyperparameter_tuning_mode:
            ...
            #copy_folder(src=optimizers_dir, dst=exp_optimizers_dir)                # TODO: remove some of these?
...
def single_forward_pass(cfg, model, train_inputs, train_features, train_label_dict, 
                        valid_endpoints_as_tensor,  loss_function, mixup_indexes_batch, mixup_lambda, logger):
    try:
    ...
    return train_outputs_dict, batch_loss, batch_per_toxicity_loss_dict, unweighted_batch_loss, unweighted_batch_per_toxicity_loss_dict

def sub_ensemble_forward_pass(cfg, model, train_inputs, train_features, train_label_dict,
                              valid_endpoints_as_tensor, loss_function, mixup_indexes_batch, mixup_lambda, logger):
    endpoint_list = cfg.endpoint_list
    device = cfg.device
    N_passes = cfg.sub_ensemble_N
    #batch_size = train_inputs.size(0)

    sub_ensemble_pred_dict = dict()
    sub_ensemble_true_dict = dict()
    mean_predictions_dict = dict()
    for endpoint in endpoint_list:
        ...
        mean_predictions_dict[endpoint] = 0

    mixup_indexes_batch_all = torch.as_tensor([], dtype=torch.int8, device=device)
    ...
    for i in range(N_passes):
        ...
        mixup_indexes_batch_all = torch.cat([mixup_indexes_batch_all, mixup_indexes_batch])
        #print(mixup_indexes_batch_all.shape)


    # for endpoint in cfg.endpoint_list:
    #     print(sub_ensemble_pred_dict[endpoint].shape)
    ...
    batch_loss, batch_per_toxicity_loss_dict, \
                unweighted_batch_loss, unweighted_batch_per_toxicity_loss_dict =  losses.calculate_loss(cfg, sub_ensemble_pred_dict, 
                                                                                                ...
                                                                                                loss_function, mixup_indexes_batch_all, mixup_lambda)
    
    
    return mean_predictions_dict, batch_loss, batch_per_toxicity_loss_dict, unweighted_batch_loss, unweighted_batch_per_toxicity_loss_dict
...
def train_DL_model(cfg, model, train_dataloader, val_dataloader, loss_function, fold_n, logger):
    ...
    # Initiate variables
    best_metrics_tracker = BestMetricsTracker(config=cfg)
    #best_val_epoch_num = 0
    #best_val_loss_value = np.inf
    #best_val_avg_auc_value = -np.inf  
    #best_val_auc_values_dict = {endpoint: -1 for endpoint in endpoint_list}  
    #best_val_loss_dict = {endpoint: np.inf for endpoint in endpoint_list}
    ...
    train_loss_values_list = list() if max_epochs > 0 else [None]              # TODO: dict of lists containing the loss values for each endpoint over all epochs (in same way as AUCs)
    ...
    optimizer = misc.get_optimizer(config=cfg, optimizer_name=cfg.optimizer_name, model=model, lr=cfg.lr, 
                                   num_batches_per_epoch=len(train_dataloader), logger=logger)
    scheduler = misc.get_scheduler(config=cfg, scheduler_name=cfg.scheduler_name, optimizer=optimizer, optimal_lr=cfg.lr,
                                   num_batches_per_epoch=len(train_dataloader), logger=logger)
    # if cfg.perform_mixup:
    #     mixup = MixUpd(keys = ['ct_dose_seg', 'features', 'label_list'], alpha=cfg.mixup_alpha, batch_size=cfg.batch_size)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch_num, batch_data in tqdm(enumerate(train_dataloader), disable=cfg.hyperparameter_tuning_mode):
            ...
            if cfg.perform_mixup:
                ...
                mixed_images, mixed_features, y_a, y_b, mixup_lambda, mixup_indexes_batch = mixup_data(cfg, train_inputs, train_features, train_label_list, cur_batch_size, alpha=cfg.mixup_alpha)
            ...
            if not cfg.sub_ensemble:
                train_outputs_dict, batch_loss, batch_per_toxicity_loss_dict, \
                unweighted_batch_loss, unweighted_batch_per_toxicity_loss_dict = single_forward_pass(cfg, model, train_inputs, train_features, train_label_dict, 
                        valid_endpoints_as_tensor,  loss_function, mixup_indexes_batch, mixup_lambda, logger)

            else:
                train_outputs_dict, batch_loss, batch_per_toxicity_loss_dict, \
                unweighted_batch_loss, unweighted_batch_per_toxicity_loss_dict = sub_ensemble_forward_pass(cfg, model, train_inputs, train_features, train_label_dict,
                              valid_endpoints_as_tensor, loss_function, mixup_indexes_batch, mixup_lambda, logger)
            ...
            #                                                                                         train_label_dict, valid_endpoints_as_tensor, 
            #                                                                                         loss_function, mixup_indexes_batch, mixup_lambda)
            # #print(batch_per_toxicity_loss_dict)
            ...
            if batch_loss.grad_fn:  # if the loss has a gradient (i.e. it is not only values for missing endpoints)
              if cfg.optimizer_name in ['ada_hessian']:  # this optimiser requires a graph to be created
                  ...
                  batch_loss.backward()
            #else:
            #    logger.my_print('No gradient for this batch. Skipping backpropagation.')
        ...
        y_pred_list_dict, y_true_list_dict = apply_sigmoid_or_softmax(cfg, train_y_pred_dict, train_y_true_dict)
        ...
        try:
            ...
            logger.my_print(y_pred_list_dict)
    ...
    # this return statement is only reached if the max_epochs is reached
...
def validate(cfg, model, dataloader, mode, loss_function, logger, save_outputs=True):
    ...
    Returns:
        loss_value: the loss on this set
        auc_value_dict: the AUC value for each endpoint
        avg_auc_value: the average AUC over all endpoints
        patient_ids: (if `mode=test`) a list of patient IDs in this set
        y_pred_list_dict: (if `mode=test`) the model's predictions on this set (a dictionary of lists, key=endpoint)
        y_true_list_dict: (if `mode=test`) the true labels for the patients (also a dictinary of lists, same structure)
    """
    endpoint_list = cfg.endpoint_list
    device = cfg.device
    ...
    with torch.no_grad():
        ...
        y_pred_list_dict, y_true_list_dict = apply_sigmoid_or_softmax(cfg, y_pred_dict, y_true_dict)
...
def report_invalid_model_trial(config):
    ...
    report = {"done" : True,
        "fold_n" : None, "epochs" : 'done', "total_params" : None,
            "train_loss" : -1, "val_loss" : -1,                         # TODO: missing average train loss over all folds!
...
def perform_N_fold_cross_validation(cfg, main_logger, hyperparameter_tuning_mode=False):
    ...
    for fold_idx in range(cv_folds): # start indexing at 1 :)
        fold_n = fold_idx + 1
        ...
        if cv_folds > 1:
            ...
            #cv_idx_list = [[train_dict, val_dict]]  # TODO: can be deleted?
        ...
        try:
            model, model_total_params = misc.get_model(config=cfg, channels=channels, depth=depth, height=height, width=width, n_features=n_features,
        ...
        if cfg.train_CITOR_models:
            ...
            test_patient_ids_lr, test_y_pred_lr_list_dict, test_y_lr_list_dict, lr_coefficients) = \
                run_logistic_regression(config=cfg, patient_id_col=data_preproc_cfg.patient_id_col,
                                        submodels_features_list=cfg.submodels_features_list,
                                        features_list=cfg.features_lr_list,
                                        lr_coefficients_list=cfg.lr_coefficients_list,
                                        #patient_ids_json=cfg.patient_ids_json,
                                        logger=logger)
                ...
                if cfg.cv_folds > 1:
                    ...
                    if (fold_n == cfg.cv_folds):
                        ...
                        make_calibration_plots(cfg, predictions_dict=ensemble_DL_predictions, true_labels_dict=test_y_true_list_dict, 
                                            lr_predictions_dict=ensemble_LR_predictions, lr_true_labels_dict=test_y_lr_list_dict, mode='calibration', set_name="Ensemble (Test Set)", 
                                            filename=os.path.join(cfg.exp_dir, 'calibration_plot_ensemble.png'))
                        DL_ens_ECE, DL_ens_MCE, LR_ens_ECE, LR_ens_MCE = make_calibration_plots(cfg, 
                                                    predictions_dict=ensemble_DL_predictions, true_labels_dict=test_y_true_list_dict, 
                                                    lr_predictions_dict=ensemble_LR_predictions, lr_true_labels_dict=test_y_lr_list_dict, mode='reliability', set_name="Ensemble (Test Set)", 
                                                    filename=os.path.join(cfg.exp_dir, 'reliability_plot_ensemble.png'))
        ...
        make_calibration_plots(cfg, predictions_dict=val_y_pred_list_dict, true_labels_dict=val_y_true_list_dict, 
                                           lr_predictions_dict=val_y_pred_lr_list_dict, lr_true_labels_dict=val_y_lr_list_dict, 
                                           mode='calibration', set_name="Validation Set", 
                                           filename=os.path.join(cfg.exp_dir, 'calibration_plot_validation.png'))
        DL_val_ECE, DL_val_MCE, LR_val_ECE, LR_val_MCE = make_calibration_plots(cfg, predictions_dict=val_y_pred_list_dict, true_labels_dict=val_y_true_list_dict, 
                                lr_predictions_dict=val_y_pred_lr_list_dict, lr_true_labels_dict=val_y_lr_list_dict, 
                                mode='reliability', set_name="Validation Set", 
                                filename=os.path.join(cfg.exp_dir, 'reliability_plot_validation.png'))
        ...
        if cfg.use_test_set:            
            make_calibration_plots(cfg, predictions_dict=test_y_pred_list_dict, true_labels_dict=test_y_true_list_dict, 
                                           lr_predictions_dict=test_y_pred_lr_list_dict, lr_true_labels_dict=test_y_lr_list_dict, 
                                           mode='calibration', set_name="Validation Set", 
                                           filename=os.path.join(cfg.exp_dir, 'calibration_plot_test.png'))
            DL_test_ECE, DL_test_MCE, LR_test_ECE, LR_test_MCE = make_calibration_plots(cfg, 
                                                    predictions_dict=test_y_pred_list_dict, true_labels_dict=test_y_true_list_dict, 
                                                    lr_predictions_dict=test_y_pred_lr_list_dict, lr_true_labels_dict=test_y_lr_list_dict, 
                                                    mode='reliability', set_name="Validation Set", 
                                                    filename=os.path.join(cfg.exp_dir, 'reliability_plot_test.png'))
            
            [DL_test_ECE_list_dict, LR_test_ECE_list_dict] = \
                misc.append_to_list_dicts(config=cfg, list_dicts =[DL_test_ECE_list_dict, LR_test_ECE_list_dict], 
                                    value_dicts = [DL_test_ECE, LR_test_ECE])
        ...
        if cfg.use_test_set:
            ...
            #dst_folder_name += '_lr_{}'.format(round(np.mean([x for x in test_auc_lr_dict.values() if x is not None]), nr_of_decimals))  # TODO: LR models?
            ...
            if fold_n == cfg.cv_folds:
                ...
                report = {"done" : True,
                    "fold_n" : fold_n, "epochs" : 'done', "total_params" : model_total_params,
                        "train_loss" : mean_train_loss, "val_loss" : mean_val_loss,                         # TODO: missing average train loss over all folds!
                ...
                #     train.report(report, checkpoint=Checkpoint.from_directory(tmpdirname))
            
            # report when not on the last fold
            else:
                # report whenever a fold is done, but it isn't the last one
                # TODO: report the mean AUC of each fold? or perhaps as a list? (add a new item to this dict for that, with a new name)
...
def ray_tune_model_hyperparameters(logger): 
    ...
    logger.my_print(f"Number of CPUs per process: {num_CPUs_per_process}")
    logger.my_print(f"Number of GPUs per process: {num_GPUs_per_process}")
...
def HP_tune_trial(hp_param_config):
    """
    this function sets up a hyperparameter tuning run, using Ray Tune
    """
    ...
    cfg = misc.make_config_object(config_file, hp_param_config=hp_param_config)

    #print("HP_tune_trial", hp_param_config.items())
    ...
    cfg = misc.rename_folders_for_parameter_tuning(cfg, new_exp_dir=new_exp_dir)
...
def main():
    import config as config_file
    cfg = misc.make_config_object(config_file, hp_param_config=None)

    # Make sure that data_preproc_cfg.use_umcg == True
    use_umcg = data_preproc_cfg.use_umcg
    assert use_umcg
...
if __name__ == '__main__':     
    ...
    to_onehot = AsDiscrete(to_onehot=config_file.num_ohe_classes)

    misc.set_random_seeds(config_file, set_new_random_seed=False)

    if config_file.reproducible:
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False
    elif config_file.cudnn_benchmark:
        torch.backends.cudnn.benchmark = True
    
    main()
    
    ## TODO: sort out the logic behind pretrained models
