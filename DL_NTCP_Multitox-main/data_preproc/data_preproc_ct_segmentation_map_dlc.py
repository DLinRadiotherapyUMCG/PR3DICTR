...
def main_segmentation_map():
    ...
    data_dir_dlc = data_prc_cfg.data_dir_dlc
    save_root_dir = data_prc_cfg.save_root_dir
    save_dir_segmentation_map_dlc = data_prc_cfg.save_dir_segmentation_map_dlc
    patient_folder_types = data_prc_cfg.patient_folder_types
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_data_preproc_ct_segmentation_map_dlc_logging_txt))
    patients_with_contouring_issues = list()
    start = time.time()

    df_mismatch_DLC_uid = pd.DataFrame()
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_dlc_list):
            ...
            if use_umcg:
                ...
                # then search in 'no_contrast'
                if len(path_rtss) == 0:
                    df_mismatch_DLC_uid_i = pd.DataFrame([folder_type_i, os.listdir(os.path.join(data_dir_dlc, patient_id))],
                                       index=['CT_folder_type_i', 'avaliable_DLC_folders'],
                                       columns=[patient_id]).T
                    df_mismatch_DLC_uid = pd.concat([df_mismatch_DLC_uid, df_mismatch_DLC_uid_i], axis=0)
                    ...
                    #print(os.path.join(data_dir_dlc, patient_id, folder_type_i_alternative))
                    all_folders_dlc_2 = get_all_folders(os.path.join(data_dir_dlc, patient_id, folder_type_i_alternative))
            ...
            with open(os.path.join(save_path, data_prc_cfg.filename_structures_value_count_json), 'w') as f:
            ...
            if len(structure_con_issues_i) > 0:
                ...
                with open(os.path.join(save_path, data_prc_cfg.filename_structures_with_contouring_issues_dlc_txt), 'w') as f:
        ...
        with open(os.path.join(save_root_dir, data_prc_cfg.filename_patients_with_contouring_issues_dlc_txt), 'w') as f:
            f.write(list_to_txt_str(patients_with_contouring_issues))

    df_mismatch_DLC_uid.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_mismatch_DLC_uid_csv), sep=';')
...
def main():
    ...
    main_overview_segmentation_map(data_dir_segmentation_map=data_prc_cfg.save_dir_segmentation_map_dlc,
                                   source='DLC',
                                   save_file=data_prc_cfg.filename_overview_structures_count_dlc_csv)
