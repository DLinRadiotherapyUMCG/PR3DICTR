    ...
    try:
        for i in range(len(ds.ROIContourSequence)):
            ...
            try:
                ...
                print(f"\nERROR HERE!, skipping {contour['name']}\n")

            #print(contour['name'], "2")
    except:
        pass
...
def get_segmentation_map(contours, slices, image, modify_structure, logger):
    ...
    # For every structure: create filled contour (and without filling holes if data_prc_cfg.unfill_holes = True) and
    ...
    for con in contours:
        ...
        #print(modify_structure(structure=con['name'], logger=logger))
        #tempprint = False
        if modify_structure(structure=con['name'], logger=logger) in data_prc_cfg.structures_uncompound_list:
            #if con['name'] == "Esophagus_Cerv": tempprint = True
            for c in con['contours']:
                try:
                    #if tempprint: print("nodes = ")
                    nodes = np.array(c).reshape((-1, 3))
                    assert np.amax(np.abs(np.diff(nodes[:, 2]))) == 0
                    # nodes[0, 2] = z-coordinate in physical space (e.g. 52.7)
                    # z_index = 2 if z = [47.7, 50.2, 52.7, ...] and nodes[0, 2] = 52.7

                    #if tempprint: print("z_index = ")
                    #if tempprint: print(z_index)
                    #if tempprint: print(nodes[0,2], nodes)
                    #if tempprint: print(z)
                    z_index = z.index(nodes[0, 2])
                    y = (nodes[:, 1] - pos_y) / spacing_y
                    x = (nodes[:, 0] - pos_x) / spacing_x

                    #if tempprint: print("z_index 2 ")
                    if data_prc_cfg.unfill_holes:
                        ...
                        image_segmentation_map_i = Image.fromarray(np.zeros_like(segmentation_map_i[:, :, z_index],
                                                                                dtype=np.uint8))
                        #if tempprint: print("draw contour outline")
                        # Draw contour outline on image_segmentation_map
                        draw = ImageDraw.Draw(image_segmentation_map_i)
                        #if tempprint: print("draw contour outline 1")
                        draw.polygon(yx, fill=0, outline=1)


                        #if tempprint: print("add contour outline to segmentation_map_i")
                    ...
                    if con['name'] == "Esophagus_Cerv":
                        # handle the exception
                        print("An exception occurred:", error) 
                    has_structure_con_issues_i = True
                    print('Error in extracting contours for structure: {name}'.format(name=con['name']))
...
def main_segmentation_map():
    ...
    # data_dir_ct = data_prc_cfg.data_dir
    # data_dir_citor = data_prc_cfg.data_dir
    save_root_dir = data_prc_cfg.save_root_dir
    save_dir = data_prc_cfg.save_dir_segmentation_map_citor
    patient_folder_types = data_prc_cfg.patient_folder_types
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_data_preproc_ct_segmentation_map_citor_logging_txt))
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        data_dir_ct = data_prc_cfg.data_dir.format(d_i)
        ...
        patients_citor_list = [x for x in patients_citor_list if '_' not in x] # Remove items with underscore from the list # DANIEL: due to new folders with underscore added in February 2024
        ...
        for patient_id in tqdm(patients_citor_list):
            ...
            if use_umcg:
                if n_path_rtss == 0:
                    folder_type_i_alternative = [x for x in patient_folder_types if x != folder_type_i][0] # grab the other folder type
                    #print(patient_folder_types)
                  
                    all_folders_citor_2 = get_all_folders(os.path.join(data_dir_citor, patient_id, folder_type_i_alternative))
            ...
            for p in path_rtss:
                ...
                for f_rtss in files_rtss:
                    ...
                    for item in ds.StructureSetROISequence:
                        ...
                        #print(item.ROIName, item.ROINumber)
                    ...
                    for value, structure in structures.items():
                        ...
                        #print(structure, value, structure_value_in_segmentation_map)
                    ...
                    with open(os.path.join(save_path, data_prc_cfg.filename_structures_value_count_i_json.format(i=i)), 'w') as f:
                    ...
                    np.save(file=os.path.join(save_path, data_prc_cfg.filename_segmentation_map_i_npy.format(i=i)),
                    ...
                    if len(structure_con_issues_i) > 0:
                        ...
                        with open(os.path.join(save_path,
                                               data_prc_cfg.filename_structures_with_contouring_issues_citor_i_txt.format(i=i)),
                                  'w') as f:
        ...
        with open(os.path.join(save_root_dir, data_prc_cfg.filename_patients_with_contouring_issues_citor_txt), 'w') as f:
            f.write(list_to_txt_str(patients_with_contouring_issues))

        # Save df with filename
        df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_patient_ids_with_filename_csv), index=False)
...
def main_best_segmentation_map():
    ...
    save_root_dir = data_prc_cfg.save_root_dir
    data_dir_segmentation_map = data_prc_cfg.save_dir_segmentation_map_citor
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_data_preproc_ct_segmentation_map_best_logging_txt))
    start = time.time()
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        patients_list = [x for x in patients_list if '_' not in x] # Remove items with underscore from the list # DANIEL: due to new folders with underscore added in February 2024
        ...
        for patient_id in tqdm(patients_list):
            ...
            for structure in data_prc_cfg.structures_uncompound_list:
                ...
                best_segmentation_map_i = np.load(
                    os.path.join(search_path, data_prc_cfg.filename_segmentation_map_i_npy.format(i=best_i)))
                ...
                if not len(best_structure_value_count_list) <= 1:
                    ...
                    assert False
                new_structure_value = data_prc_cfg.structures_values[structure]
                if len(best_structure_value_count_list) == 1:
                    ...
                    output_segmentation_map = add_segmentation_maps(
                        ...
                        keep_segmentation_map_elements=data_prc_cfg.keep_segmentation_map_elements, logger=logger)
                else:
            ...
            with open(os.path.join(save_path, data_prc_cfg.filename_best_count_files_json), 'w') as f:
            ...
            np.save(file=os.path.join(save_path, data_prc_cfg.filename_segmentation_map_npy), arr=output_segmentation_map)
...
def main_overview_all_segmentation_map():
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_overview_all_structures_count_citor_csv), sep=';')
...
def main_overview_segmentation_map(data_dir_segmentation_map, source, save_file):
    ...
    save_root_dir = data_prc_cfg.save_root_dir
    structures_uncompound_list = data_prc_cfg.structures_uncompound_list
    start = time.time()
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_list):
            ...
            segmentation_map_file = [x for x in all_patient_files if x == data_prc_cfg.filename_segmentation_map_npy]
            ...
            if len(segmentation_map_file) == 1:
                ...
                for structure in structures_uncompound_list:
                    structure_count = np.sum(segmentation_map == data_prc_cfg.structures_values[structure])
...
def main_patients_get_dlc():
    ...
    This function copy-paste data_prc_cfg.data_dir's patient CT folders to local folder. Those folders should be uploaded
...
def main():
    ...
    main_overview_segmentation_map(data_dir_segmentation_map=data_prc_cfg.save_dir_segmentation_map_citor,
                                   source='',
                                   save_file=data_prc_cfg.filename_overview_structures_count_citor_csv)
