...
def main_copy():
    ...
    save_dir_mode_list = data_prc_cfg.save_dir_mode_list
    patient_id_length = data_prc_cfg.patient_id_length

    data_collection_dir = data_prc_cfg.data_collection_dir
    filename_patient_ids = data_prc_cfg.filename_patient_ids
    logger = Logger()
    start = time.time()
    ...
    for m_i, d_i in zip(mode_list, save_dir_mode_list):
        ...
        dst = data_prc_cfg.data_dir.format(d_i)
...
def main_frame_of_reference_uid():
    ...
    frame_of_ref_uid_tag = data_prc_cfg.frame_of_ref_uid_tag
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        if use_umcg:
            df_data_folder_types = pd.read_csv(os.path.join(save_root_dir, data_prc_cfg.filename_patient_folder_types_csv),
                                               sep=';',
                                               index_col=0)
        ...
        # Save to csv
        df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_frame_of_reference_uid_csv), sep=';', index=False)
...
def main_match_ct_rtdose_rtstruct():
    ...
    patient_folder_types = data_prc_cfg.patient_folder_types
    frame_of_ref_uid_tag = data_prc_cfg.frame_of_ref_uid_tag
    patient_id_tag = data_prc_cfg.patient_id_tag
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        # Save to csv
        df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_invalid_uid_csv), sep=';', index=False)
