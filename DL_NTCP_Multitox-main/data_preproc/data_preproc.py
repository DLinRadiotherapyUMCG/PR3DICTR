...
IMPORTANT NOTES:
    - We apply 'perform_spacing_correction=not data_prc_cfg.perform_spacing_correction', i.e. we do not apply
...
def cropping(arr, cropping_region, z_size, y_size, x_size):
    ...
    y_center_shift = data_prc_cfg.y_center_shift_perc * int(cropping_region['y_num_rows'])
...
def transformation(arr, resize_mode):
    ...
    """
    xform = Compose([
        Resize(data_prc_cfg.input_size, mode=resize_mode)
    ])
    return xform(arr)
...
def get_single_channel_array(arr_path, metadata, is_label, cropping_region, bb_size,
    ...
    if perform_spacing_correction:
        arr = spacing_correction(arr=arr, metadata=metadata, out_spacing=data_prc_cfg.spacing, is_label=is_label)
        if logger:
    ...
    if perform_clipping:
        ...
        arr = np.clip(arr, a_min=data_prc_cfg.ct_min, a_max=data_prc_cfg.ct_max)
        if logger:
    ...
    if perform_transformation:
        ...
        if is_label:
            resize_mode = data_prc_cfg.label_resize_mode
        else:
            resize_mode = data_prc_cfg.non_label_resize_mode
...
def process_main_arrays():
    ...
    save_dir_ct = data_prc_cfg.save_dir_ct
    save_dir_rtdose = data_prc_cfg.save_dir_rtdose
    save_dir_segmentation_map = data_prc_cfg.save_dir_segmentation_map
    save_dir_dataset_full = data_prc_cfg.save_dir_dataset_full
    patient_id_col = data_prc_cfg.patient_id_col
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_data_preproc_array_logging_txt))
    start = time.time()
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        if data_prc_cfg.perform_cropping:
            cropping_regions = pd.read_csv(os.path.join(save_root_dir, data_prc_cfg.filename_cropping_regions_csv),
                                           sep=';', index_col=0)
        ...
        #    process_patient_array(logger, df_data_folder_types, cropping_regions, patient_id)
        
        with Pool(processes = data_prc_cfg.workers) as pool:
            results = tqdm(pool.imap(multiproc_process_patient_array, zip(itertools.repeat(logger), itertools.repeat(df_data_folder_types), itertools.repeat(cropping_regions), patients_list)), total=len(patients_list))
            tuple(results)
...
def multiproc_process_patient_array(args):
    """Convert `f([1,2])` to `f(1,2)` call."""
    return process_patient_array(*args)

def process_patient_array(logger, df_data_folder_types, cropping_regions, patient_id):

    save_dir_ct = data_prc_cfg.save_dir_ct
    save_dir_segmentation_map = data_prc_cfg.save_dir_segmentation_map
    save_dir_dataset_full = data_prc_cfg.save_dir_dataset_full
    bb_size = data_prc_cfg.bb_size


    logger.my_print('Patient_id: {id}'.format(id=patient_id))
    if data_prc_cfg.workers > 1:  logger = None # suppress the logger for the info during each step
    ...
    ct_metadata = None
    if data_prc_cfg.perform_spacing_correction:
        ct_metadata_path = os.path.join(save_dir_ct, patient_id, data_prc_cfg.filename_ct_metadata_json)
        ...
        if logger != None:
            ...
            logger.my_print('\tdata_prc_cfg.spacing[::-1] (output): {}'.format(data_prc_cfg.spacing[::-1]))
    ...
    if logger != None: logger.my_print('\t----- CT -----')
    ct_arr_path = os.path.join(save_dir_ct, patient_id, data_prc_cfg.filename_ct_npy)
    ct_arr = get_single_channel_array(arr_path=ct_arr_path, dtype=None, metadata=ct_metadata, is_label=False,
                                              ...
                                              bb_size=bb_size,
                                              perform_spacing_correction=data_prc_cfg.perform_spacing_correction,
                                              perform_cropping=data_prc_cfg.perform_cropping,
                                              perform_clipping=data_prc_cfg.perform_clipping,
                                              perform_transformation=data_prc_cfg.perform_transformation, logger=logger)
    ...
    if logger != None: logger.my_print('\t----- RTDOSE -----')
    resampled_rtdose_arr_path = os.path.join(save_dir_ct, patient_id, data_prc_cfg.filename_rtdose_npy)
    resampled_rtdose_arr = get_single_channel_array(arr_path=resampled_rtdose_arr_path, dtype='int16',
                                                            ...
                                                            perform_transformation=data_prc_cfg.perform_transformation,
                                                            logger=logger)
            ...
            # Note: we apply 'perform_spacing_correction=not data_prc_cfg.perform_spacing_correction', i.e. we do not apply
            # spacing correction here if it has been applied in data_preproc_ct_segmentation_map.py, and vice versa.
    if logger != None: logger.my_print('\t----- Segmentation_map -----')
    segmentation_map_arr_path = os.path.join(save_dir_segmentation_map, patient_id,
                                                     data_prc_cfg.filename_segmentation_map_npy)
    segmentation_map_arr = get_single_channel_array(arr_path=segmentation_map_arr_path, dtype='int16',
                                                            ...
                                                            cropping_region=cropping_region_i,
                                                            bb_size=bb_size,
                                                            perform_spacing_correction=not data_prc_cfg.perform_spacing_correction,
                                                            perform_cropping=data_prc_cfg.perform_cropping,
                                                            perform_clipping=False,
                                                            perform_transformation=data_prc_cfg.perform_transformation,
                                                            logger=logger)
...
def copy_files_to_final_dataset():
    logger = Logger(output_filename=None)
    start = time.time()

    # move all of the preprocessed image files to the final dataset
    # (they can be copied from the 'dataset_full' folder)
    dir_dataset_images_subfolder =  os.path.join(data_prc_cfg.save_dir_final_dataset, data_prc_cfg.foldername_images)
    create_folder_if_not_exists(dir_dataset_images_subfolder)

    patient_image_folders = next(os.walk(data_prc_cfg.save_dir_dataset_full))[1]

    # copy each patient's folder 
    # (done with a for loop over each patient, rather than just copy the whole folder, for the progress bar)
    logger.my_print(f"Moving all image (.npy) files to {dir_dataset_images_subfolder}")

    
    for patient in tqdm(patient_image_folders):
        src = os.path.join(data_prc_cfg.save_dir_dataset_full, patient)
        dst = os.path.join(dir_dataset_images_subfolder, patient)

        copy_folder(src, dst)

    # Copy exclude_patients to final dataset folder
    logger.my_print("Moving exclude_patients to output dataset")
    src = os.path.join(data_prc_cfg.save_root_dir, data_prc_cfg.filename_exclude_patients_csv)
    dst = os.path.join(data_prc_cfg.save_dir_final_dataset, data_prc_cfg.filename_exclude_patients_csv)
    copy_file(src, dst)
...
def main():
    ...
    # preprocess the image arrays (i.e. crop them down to 100x100x100)
    process_main_arrays()

    # preprocess the (non-image) features
    data_preproc_features.main()

    # move images to dataset destination
    copy_files_to_final_dataset()
