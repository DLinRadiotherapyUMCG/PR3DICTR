...
def main_ct(df, data_dir, patient_id, folder_type, src, logger):
    ...
    if len(all_folders) > 0:
        ...
        if len(path_ct) == 1:
            ...
            path_ct = path_ct[0]
            files_dcm_ct = glob(path_ct + '/*{}'.format(data_prc_cfg.ct_file_ext))
...
def main_rtstruct(df, data_dir, patient_id, folder_type, src, logger):
    ...
    if len(all_folders) > 0:
        ...
        if not len(path_rtstruct) == 1:
            ...
            files_dcm_rtstruct = glob(path_rtstruct + '/*{}'.format(data_prc_cfg.rtstruct_file_ext))
...
def main():
    ...
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_check_folder_structure_logging_txt))
    start = time.time()
    ...
    for m_i, d_i in zip(mode_list, data_dir_mode_list):
        ...
        df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_patients_incorrect_folder_structure_csv.format(m_i)),
                  sep=';', index=False)
