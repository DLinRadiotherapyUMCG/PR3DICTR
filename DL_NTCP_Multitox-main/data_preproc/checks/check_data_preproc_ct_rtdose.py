...
def main():
    ...
    save_dir = data_prc_cfg.save_dir_ct
    ct_json_filename = data_prc_cfg.filename_ct_metadata_json
    rtdose_json_filename = data_prc_cfg.filename_rtdose_metadata_json
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_check_data_preproc_ct_rtdose_logging_txt))
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_list):
            ...
            # Load CT
            ct = np.load(os.path.join(save_dir, patient_id, data_prc_cfg.filename_ct_npy))
            ...
            # Load RTDOSE
            rtdose = np.load(os.path.join(save_dir, patient_id, data_prc_cfg.filename_rtdose_npy))
        ...
        # Save df to csv file
        ct_metadata_df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_overview_ct_metadata_csv), sep=';')
        ct_df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_overview_ct_csv), sep=';')
        rtdose_df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_overview_rtdose_csv), sep=';')
