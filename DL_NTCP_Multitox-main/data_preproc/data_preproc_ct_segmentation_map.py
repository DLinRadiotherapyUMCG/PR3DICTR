...
This script:
    - main_segmentation_map(): For each patient:
        ...
        - Perform spacing_correction if data_prc_cfg.perform_spacing_correction = True.
...
IMPORTANT NOTES:
    ...
    - overview_segmentation_map will be created for spacing corrected segmentation_map (if
        data_prc_cfg.perform_spacing_correction = True), therefore the counts may be slightly different from those in
        overview_segmentation_map_citor and overview_segmentation_map_dlc!
    - In data_preproc.py we apply 'perform_spacing_correction=not data_prc_cfg.perform_spacing_correction', i.e.
...
def get_subset_segmentation_map(segmentation_map, structures, structures_value_dict, as_binary, logger):
    ...
    for structure in structures:
        ...
        if structure_count >= data_prc_cfg.minimal_count:
            ...
            logger.my_print('\t{structure} has count = {count} < {minimal_count}'
                            .format(structure=structure,
                                    ...
                                    minimal_count=data_prc_cfg.minimal_count),
                            level='warning')
...
def main_segmentation_map():
    ...
    data_dir_ct = data_prc_cfg.save_dir_ct
    save_root_dir = data_prc_cfg.save_root_dir
    data_dir_segmentation_citor = data_prc_cfg.save_dir_segmentation_map_citor
    data_dir_segmentation_dlc = data_prc_cfg.save_dir_segmentation_map_dlc
    save_dir_segmentation_map = data_prc_cfg.save_dir_segmentation_map
    minimal_count = data_prc_cfg.minimal_count
    keep_segmentation_map_elements = data_prc_cfg.keep_segmentation_map_elements
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        invalid_uid = pd.read_csv(os.path.join(save_root_dir, data_prc_cfg.filename_invalid_uid_csv), sep=';', index_col=0)
        ...
        invalid_uid_patient_ids = ['%0.{}d'.format(data_prc_cfg.patient_id_length) % x for x in invalid_uid.index]
        ...
        for patient_id in tqdm(patients_list):
            ...
            for structure in data_prc_cfg.structures_uncompound_list:
                structure_value = data_prc_cfg.structures_values[structure]
            ...
            # Note: in data_preproc.py we apply 'perform_spacing_correction=not data_prc_cfg.perform_spacing_correction', i.e.
            # do not apply spacing correction in data_preproc.py if it has been applied here, and vice versa.
            if data_prc_cfg.perform_spacing_correction:
                # Load ct_metadata
                ct_metadata_path = os.path.join(data_dir_ct, patient_id, data_prc_cfg.filename_ct_metadata_json)
                ...
                output_segmentation_map = spacing_correction(arr=output_segmentation_map, metadata=ct_metadata,
                                                             out_spacing=data_prc_cfg.spacing,
                                                             is_label=True)
...
def main_cropping_regions():
    ...
    save_root_dir = data_prc_cfg.save_root_dir
    data_dir_segmentation_map = data_prc_cfg.save_dir_segmentation_map
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_data_preproc_ct_segmentation_map_cropping_logging_txt))
    nr_of_decimals = data_prc_cfg.nr_of_decimals
    start = time.time()
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_list):
            ...
            z_values_filter = [data_prc_cfg.structures_values[x] for x in
                               data_prc_cfg.parotis_structures + data_prc_cfg.lower_structures + data_prc_cfg.mandible_structures]
            # z_values_filter = [data_prc_cfg.structures_values[x] for x in data_prc_cfg.structures_uncompound_list]
            # mask.shape = (z, y, x)
            ...
            y_values_filter = [data_prc_cfg.structures_values[x] for x in data_prc_cfg.parotis_structures]
            # y_values_filter = [data_prc_cfg.structures_values[x] for x in data_prc_cfg.structures_uncompound_list]
            ...
            x_values_filter = [data_prc_cfg.structures_values[x] for x in data_prc_cfg.parotis_structures + data_prc_cfg.submand_structures]
            # x_values_filter = [data_prc_cfg.structures_values[x] for x in data_prc_cfg.structures_uncompound_list]
        ...
        cropping_regions_dict_df.to_csv(os.path.join(save_root_dir, data_prc_cfg.filename_cropping_regions_csv), sep=';')
...
def main():
    ...
    main_overview_segmentation_map(data_dir_segmentation_map=data_prc_cfg.save_dir_segmentation_map,
                                   source='any',
                                   save_file=data_prc_cfg.filename_overview_structures_count_csv)
