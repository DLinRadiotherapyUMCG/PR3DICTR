...
def load_ct(path_ct, logger):
    ...
    arr_ct = arr_ct.astype(data_prc_cfg.ct_dtype)
...
def main():
    ...
    # data_dir = data_prc_cfg.data_dir
    save_root_dir = data_prc_cfg.save_root_dir
    save_dir_rtdose = data_prc_cfg.save_dir_rtdose
    save_dir_ct = data_prc_cfg.save_dir_ct
    logger = Logger(os.path.join(save_root_dir, data_prc_cfg.filename_data_preproc_ct_rtdose_logging_txt))
    # filename_rtdose = data_prc_cfg.filename_rtdose_dcm
    start = time.time()
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        #    preprocess_patient_ct_and_rtdose(logger, data_dir, df_data_folder_types, patient_id)

        with Pool(processes = data_prc_cfg.workers) as pool:
            results = tqdm(pool.imap(multiproc_preprocess_patient_ct_and_rtdose, zip(itertools.repeat(logger), itertools.repeat(data_dir), itertools.repeat(df_data_folder_types), patients_list)), total=len(patients_list))
            tuple(results)
...
def multiproc_preprocess_patient_ct_and_rtdose(a_b_c):
    """Convert `f([1,2])` to `f(1,2)` call."""
    return preprocess_patient_ct_and_rtdose(*a_b_c)

def preprocess_patient_ct_and_rtdose(logger, data_dir, df_data_folder_types, patient_id):
    use_umcg = data_prc_cfg.use_umcg
    save_dir_rtdose = data_prc_cfg.save_dir_rtdose
    save_dir_ct = data_prc_cfg.save_dir_ct
    patient_folder_types = data_prc_cfg.patient_folder_types

    show_logger = True if data_prc_cfg.workers == 0 else False
    
    if show_logger: logger.my_print('Patient_id: {}'.format(patient_id))
    ...
    if use_umcg:
        ...
        folder_type_i = ''
            ...
            #####
    if show_logger: logger.my_print('\tSTEP 1 (RTDOSE, CT) - Fetching folder and files...')
    ...
    path_rtdose_file = os.path.join(path_rtdose, rtdose_file[0])
            ...
            #####
    if show_logger: logger.my_print('\tSTEP 2 (RTDOSE, CT) - Loading Pydicom Dataset (RTDOSE) and Images (RTDOSE, CT)...')
    ds = dcmread(path_rtdose_file)
    image_ct, _ = load_ct(path_ct=path_ct, logger=logger)
    image_rtdose = load_rtdose(path_rtdose)

            #####
    if show_logger: logger.my_print('\tSTEP 3 (RTDOSE, CT) - Extracting meta-data: general...')
            # CT
    ds_ct_dict = dict()
    ...
    ds_rtdose_dict['Dose_units'] = ds.DoseUnits
            ...
            #####
    if show_logger: logger.my_print('\tSTEP 4 (RTDOSE) - Resampling Image...')
    ...
    arr_rtdose = (arr_rtdose * ds_rtdose_dict['Dose_grid_scaling'] * 100).astype(data_prc_cfg.rtdose_dtype)
    resampled_arr_rtdose = (resampled_arr_rtdose * ds_rtdose_dict['Dose_grid_scaling'] * 100).astype(
                data_prc_cfg.rtdose_dtype)

            #####
    if show_logger: logger.my_print('\tSTEP 5 (RTDOSE) - Extracting meta-data: before data processing and after data processing...')
    ...
    for key, im, arr in zip(meta_data_list, image_rtdose_list, arr_rtdose_list):
        ...
        ds_rtdose_dict[key] = ds_rtdose_dict_i
            ...
            #####
    if show_logger: logger.my_print('\tSTEP 6 (RTDOSE, CT) - Saving meta-data and arrays...')
    ...
    with open(os.path.join(save_path_rtdose, data_prc_cfg.filename_rtdose_metadata_json), 'w') as file:
        json.dump(ds_rtdose_dict, file, default=set_default)

    with open(os.path.join(save_path_ct, data_prc_cfg.filename_ct_metadata_json), 'w') as file:
        json.dump(ds_ct_dict, file, default=set_default)

            # Save as Numpy array
    np.save(file=os.path.join(save_path_ct, data_prc_cfg.filename_ct_npy), arr=arr_ct)
    np.save(file=os.path.join(save_path_rtdose, data_prc_cfg.filename_rtdose_npy), arr=resampled_arr_rtdose)
