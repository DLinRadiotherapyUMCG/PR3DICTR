...
from data_preproc_functions import get_all_combinations, create_folder_if_not_exists
import checks.plotting_functions as plt_fncs 
# Whether to preprocess UMCG data or other data
use_umcg = True

# Whether to apply multiprocessing to certain steps:
workers = os.cpu_count() # 1 is normal use (1 core) 
                 # os.cpu_count()   gives the highest possible number of threads (i.e. the fastest, but more intensive)
            # >1 will disable many of the logging outputs, as they are no longer shown in the right order. In case of debugging, probably best to set to 1.

# Paths
# TODO: temporary, for MDACC
if use_umcg:
    ##  INPUTS/SOURCES  ##
    # Toxicity/clinical speadsheet, and dose info spreadsheet paths
    toxicity_endpoints_spreadsheet_file_path = "//zkh/appdata/RTDicom/Projectline_HNC_modelling/Users/Daniel MacRae/CITOR_REDCAP_clinical_data_important_variables_combined.xlsx"
    dose_spreadsheet_file_path = "//zkh/appdata/RTDicom/Projectline_HNC_modelling/Users/Daniel MacRae/extracted_dose_16_5_2022.csv"
    ...
    ##  OUTPUTS  ##
    save_dir_citor = 'H:/preprocess_MT_data_UID_FULL/raw_rtstructs_citor'  # (Optional, only used if `check_data.py` is in use) Saves a copy of all of the CITOR RTSTRUCTs here
    save_root_dir = 'H:/preprocess_MT_data_UID_FULL'  # Contains saved files from scripts such as processed data, loggings, etc.
    save_root_dir_2 = 'H:/HNC_MT_Data_UID_FULL'  # Contains saved files for dataset

    # NOTE: THESE ARE THE OLD ONES!
    # save_dir_citor = 'H:/preprocess_multitox/raw_rtstructs_citor'  # (Optional, only used if `check_data.py` is in use) Saves a copy of all of the CITOR RTSTRUCTs here
    ...
    # save_dir_citor = 'H:/preprocess_MT_data/raw_rtstructs_citor'  # (Optional, only used if `check_data.py` is in use) Saves a copy of all of the CITOR RTSTRUCTs here
    # save_root_dir = 'H:/preprocess_MT_data'  # Contains saved files from scripts such as processed data, loggings, etc.
    # save_root_dir_2 = 'H:/HNC_MT_Data'  # Contains saved files for dataset
    save_dir = os.path.join(save_root_dir_2, 'dicom_processed')

    # directory to where the final dataset should go (i.e. where your code is)
    save_dir_final_dataset = "//zkh/appdata/RTDicom/Projectline_HNC_modelling/Users/Daniel MacRae/DL_NTCP_Multitox/datasets/MT_dataset"
#os.path.join(save_root_dir, 'dataset_dan')
...
if use_umcg:
    ...
    save_dir_figures_all = os.path.join(save_root_dir, 'figures_all')
else:
    # data_preproc_segmentation_map_citor.py
...
filename_mismatch_DLC_uid_csv = 'mismatch_dlc_uid.csv'
# data_preproc_ct_segmentation_map.py
...
filename_multihot_encoded_segmentation_map_npy = 'multihot_encoded_segmentation_map.npy'
...
# data_preproc_features
image_file_keys = [filename_ct_npy, filename_rtdose_npy, filename_segmentation_map_npy]
filname_features_spreadsheet_csv = 'patients_all_features.csv'
filname_features_spreadsheet_excel = 'patients_all_features.xlsx'
            
# main.py
filename_main_logging_txt = 'main_logging.txt'


# Modelling/Feature preprocessing    (data_preproc_features.py)
# all of these features will be written into 'filname_features_spreadsheet_csv' in the final dataset folder
# they don't necessarily all need to be used in the main model code, you can pick andf chose in config.py
# this just makes the spreadsheet with everything you might need
patient_id_col = 'PatientID'
clinical_features = [patient_id_col] + ["Sex", "Age", "CT+C_available", "CT_Artefact", "Loctum2", "Photons", "T_stage", "N_stage", "Smoking", "P16", "WHO"] 

target_toxicities = ["Xerostomia", "Aspiration", "Sticky", "Taste", "Dysphagia"] 
baseline_timepoints = ["BSL", "W01"]  # which baseline to use ["BSL", "W01"]
target_timepoints = ["M06", "M12", "M18"]    # what the endpoint time we're trying to predict is
                               # ["M06", "M12", "M18"]    
...
toxicity_baseline_columns = get_all_combinations(target_toxicities, baseline_timepoints, spacer="_")
toxicity_endpoint_columns = get_all_combinations(target_toxicities, target_timepoints, spacer="_")

## what to keep from `dose_spreadsheet_file_path` file 
...
#colormapping_dict = plt_fncs.HNC_color_mapping_dict
rtdose_cmap, rtdose_norm, rtdose_labels = plt_fncs.create_RTDOSE_cmap("HNC")
#= colors.LinearSegmentedColormap.from_list('rtdose_cmap', np.vstack((rtdose_cmap_lower, rtdose_cmap_mid,
#                                                                                 rtdose_cmap_upper)))
segmentation_cmap = 'gray'
draw_contour_structures = structures_uncompound_list  # create contour of these structures in the figures
draw_contour_color = 'magenta'  # color of the contour line
draw_contour_linewidth = 0.25  # width of the contour line
ct_ticks_steps = 100  # optional (else: None)
rtdose_ticks_steps = rtdose_labels # 1000  # optional (else: None)
...
test_patients_list = None #['0359912'] # ['0137534', "0507025", "0851961", "2067099"]
# [ "0005680", "0020715", "0021879",  # these three are known to be fine ! 
#                       "0507025", "0851961", "2067099", "2102777", "2146277", "3144104", "3941688", "4673928", "5088300", "5896858", \
#                       "0359912", "3573430", "5052615"]

#None # ["0359912", "1299897", "2022769", "3573430", "5052615"] # ["0005680", "5052615"]
#["0005680", "0359912", "1299897", "2022769", "2103749", "3573430", "5052615"]  # None 

#test_patients_list = ["0891491", '2146277', '3144104', "8350445"]
#test_patients_list = ["2067099", "2102777", "3144104"]
