    ...
    def forward(self, x, features, vectorize=False):
       ...
       # print(x.shape, features.shape)
        ...
        if self.skip_connection:
            x = torch.concatenate((i_attention_block, f_attention_block, x, features), dim=-1)
        else:
...
#         self.attention = nn.MultiheadAttention(input_dim, num_heads=num_heads, dropout=dropout_rate)
