...
class ResNet_LReLU(nn.Module):
    ...
    def __init__(self,
        ...
        self.first_conv_layer = nn.ModuleList()
        for idx in range(n_input_channels):
            first_conv_block_layer = nn.Sequential(
                nn.Conv3d(1,
                            self.in_planes,
                ...
                nn.LeakyReLU(negative_slope=lrelu_alpha),
                
            )
            
            self.first_conv_layer.add_module('conv_block_1_%s' % idx, first_conv_block_layer)
    ...
    def forward(self, x, autoencoder=False):  # features are also passed to this function, just for consistency of the forward functions
        """
        ...
        # x = self.bn1(x)
        # x = self.act(x)
        # first block (DANIEL: new style of seperated feature maps per input channel. Feature maps are summed up in the end)
        #x = sum[block(x[:,idx,:,:,:]) for (idx, block) in enumerate(self.first_conv_layer)]


        feature_maps = None
        for idx, block in enumerate(self.first_conv_layer):
            #print(idx)
...
def get_resnet_lrelu(config, model_depth, channels, n_features, filters, lrelu_alpha, **kwargs):
    """
    ResNet
    ...
    if model_depth == 10:
        model = ResNet_LReLU(config, BasicBlock, [1, 1, 1, 1], block_inplanes=filters, n_input_channels=channels, 
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
    elif model_depth == 18:
        model = ResNet_LReLU(config, BasicBlock, [2, 2, 2, 2], block_inplanes=filters, n_input_channels=channels, 
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
    elif model_depth == 34:
        model = ResNet_LReLU(config, BasicBlock, [3, 4, 6, 3], block_inplanes=filters, n_input_channels=channels, 
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
    elif model_depth == 50:
        model = ResNet_LReLU(config, Bottleneck, [3, 4, 6, 3], block_inplanes=filters, n_input_channels=channels,
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
    elif model_depth == 101:
        model = ResNet_LReLU(config, Bottleneck, [3, 4, 23, 3], block_inplanes=filters, n_input_channels=channels, 
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
    elif model_depth == 152:
        model = ResNet_LReLU(config, Bottleneck, [3, 8, 36, 3], block_inplanes=filters, n_input_channels=channels, 
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
    elif model_depth == 200:
        model = ResNet_LReLU(config, Bottleneck, [3, 24, 36, 3], block_inplanes=filters, n_input_channels=channels, 
                             n_features=n_features, lrelu_alpha=lrelu_alpha, **kwargs)
...
def get_resnet(config, model_depth, channels, n_features, filters, lrelu_alpha, **kwargs):
    """
    ResNet
