...
class ECB(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.conv(out) # (B, dim, 14, 14, 14)
        return x
...
class MedViT(nn.Module):
    def __init__(self, config, n_features, channels, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        self.stem = nn.Sequential(
            ConvBNReLU(channels, stem_chs[0], kernel_size=3, stride=2),
    ...
    def _initialize_weights(self):
        for n, m in self.named_modules():
            if isinstance(m, (nn.BatchNorm3d, nn.GroupNorm, nn.LayerNorm, nn.BatchNorm1d)):
                ...
                if not isinstance(m, nn.LazyLinear):  # lazylinear is used in the output head, use this if-statement to avoid weight initialisation error
...
@register_model
def MedViT_small(config, n_features, channels, pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(config, n_features, channels, stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1, **kwargs)
    return model


@register_model
def MedViT_base(config, n_features, channels, pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(config, n_features, channels, stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, **kwargs)
    return model


@register_model
def MedViT_large(config, n_features, channels, pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(config, n_features, channels, stem_chs=[64, 32, 64], depths=[3, 4, 30, 3], path_dropout=0.2, **kwargs)
    return model
