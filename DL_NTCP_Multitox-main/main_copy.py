...
import misc
import utils.losses.main as losses
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch_num, batch_data in tqdm(enumerate(train_dataloader), disable=cfg.hyperparameter_tuning_mode):
            ...
            if cfg.model_name == "MLP":
                train_features, train_label_list = (
            ...
            if ((epoch == 1) or (epoch % cfg.plot_interval == 0)) and batch_num==0 and cfg.model_name != "MLP":
...
def validate(cfg, model, dataloader, mode, loss_function, logger, save_outputs=True):
    ...
    #loss_function = losses.get_loss_function(config=cfg)
...
def perform_N_fold_cross_validation(cfg, main_logger, hyperparameter_tuning_mode=False):
    ...
    if cv_folds > 1:
        ...
        if cfg.cv_type == 'stratified':
            ...
            # DANIEL: added roundabout way to do strat sampling with multi-label data
    ...
    for fold_idx in range(cv_folds): # start indexing at 1 :)
            ...
            if cfg.use_test_set:
                ...
                if cfg.cv_folds > 1:
                    ...
                    if (fold_n == cfg.cv_folds):
                        ...
                        [DL_ens_ECE, LR_ens_ECE], [DL_ens_MCE, LR_ens_MCE] = adaptive_make_calibration_plots(cfg, ensemble_calibration_plot_dict_list, column_names = cfg.endpoint_list, mode='reliability',
        ...
        [DL_val_ECE, LR_val_ECE], [DL_val_MCE, LR_val_MCE] = adaptive_make_calibration_plots(cfg, val_calibration_plot_dict_list, column_names = cfg.endpoint_list, mode='reliability',
        ...
        if cfg.use_test_set:      
            ...
            make_calibration_plots(cfg, predictions_dict=test_y_pred_list_dict, true_labels_dict=test_y_true_list_dict, 
                                           ...
                                           mode='calibration', set_name="Test Set", 
            ...
            DL_test_ECE, DL_test_MCE, LR_test_ECE, LR_test_MCE = make_calibration_plots(cfg, 
                                                    ...
                                                    lr_predictions_dict=test_y_pred_lr_list_dict, lr_true_labels_dict=test_y_lr_list_dict, 
                                                    mode='reliability', set_name="Test Set", 
            ...
            [DL_test_ECE, LR_test_ECE], [DL_test_MCE, LR_test_MCE] = adaptive_make_calibration_plots(cfg, test_calibration_plot_dict_list, column_names = cfg.endpoint_list, mode='reliability',
